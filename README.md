...
- [`emergency-intake-practice`](skills/emergency-intake-practice/) - Experimental, preview-first practice call for the citizen side of an emergency call: a self-identifying AI demo (never the real service) runs a one-question-at-a-time intake with a named, consenting participant and returns a structured result with urgency and caller clarity; real emergencies are directed to the real number; ships a zero-live-call test suite and fixture replay so every safety line is verifiable offline.
